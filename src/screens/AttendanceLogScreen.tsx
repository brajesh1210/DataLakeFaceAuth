...
function filterRecords(
  ...
  switch (filter) {
    case 'today':
      return records.filter(r => new Date(r.timestamp).toDateString() === todayStr);
    case 'yesterday': {
...
export const AttendanceLogScreen = ({ navigation }: TabScreenProps<'Log'>) => {
        ...
        <View style={styles.recordRow}>
          ...
          <View style={styles.recordInfo}>
            ...
            <Text style={[typography.caption, { color: colors.text.secondary }]}>
              {item.employeeId} • {item.type === 'check-in' ? 'In' : 'Out'} {formatTime(item.timestamp)}
            </Text>
          </View>
          <View style={styles.recordRight}>
            ...
            />
            <Icon name="chevron-right" size={20} color={colors.text.tertiary} style={{ marginLeft: 6 }} />
          </View>
        </View>
      </Card>
    ),
  ...
  const EmptyState = () => (
    <View style={styles.emptyContainer}>
      ...
      <Text style={[typography.h3, { color: colors.primary.navy, marginBottom: 8 }]}>
        No records found
      </Text>
      <Text style={[typography.body, { color: colors.text.secondary, textAlign: 'center' }]}>
        No attendance records match the selected filter. Try a different time range.
      </Text>
    </View>
  );
            ...
            >
              <Text
                style={[
                  ...
                  {
                    color: isActive ? colors.text.white : colors.primary.navy,
                    fontWeight: '600',
                  },
                ]}
              >
